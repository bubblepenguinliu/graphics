...
void Rasterizer::rasterize_triangle(Triangle& t)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        w[i] = {
            t.world_pos[i].x(), t.world_pos[i].y(), t.world_pos[i].z()
        ...
        for (int y = y_start; y <= y_end; y++) {
            if (inside_triangle(x, y, t.viewport_pos)) {
                ...
                auto interpolated_world_pos =
                    interpolate(alpha, beta, gamma, w[0], w[1], w[2], weight, Z);
                auto interpolated_normal =
                    interpolate(alpha, beta, gamma, n[0], n[1], n[2], weight, Z).normalized();
